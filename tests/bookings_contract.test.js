...
assert.match(server, /bookingId/);
assert.match(server, /jobId/);
assert.match(server, /clientId/);
assert.match(server, /recommendation/);
assert.match(server, /nextStep/);
assert.match(server, /Booking saved\. Recommendation pending/);
...
assert.ok(/if \(options\.advance\) \{\s*setStep\(1\);/s.test(js), 'package selection keeps required client details step active');
