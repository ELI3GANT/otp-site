...
(function() {
    ...
    // 9. NEURAL CONSOLE LOGIC
    window.toggleNeuralConsole = function() {
        const c = document.getElementById('neuralConsole');
        const i = document.getElementById('consoleInput');
        if(!c) return;
    ...
    const logToConsole = (msg, type='system') => {
        const log = document.getElementById('consoleLog');
        if(!log) return;
        const d = document.createElement('div');
        d.className = `console-msg ${type}`;
        
        // Simple parsing
        if(type === 'ai') {
             msg = msg.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>'); // Bold
             msg = msg.replace(/`(.*?)`/g, '<code style="background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:3px;">$1</code>');
             msg = msg.replace(/\n/g, '<br>');
        }
    ...
    function handleSystemCommand(cmd) {
        ...
        if(c === '/help') {
             logToConsole("COMMANDS: <br>/clear - Clear Log <br>/sys - System Status <br>/ping - Test Latency", 'system');
        } else if(c === '/clear') {
            const log = document.getElementById('consoleLog');
            if(log) log.innerHTML = '';
            logToConsole("LOG CLEARED", 'system');
        } else if(c === '/sys') {
             logToConsole(`CPU: ${navigator.hardwareConcurrency || '?'} CORES <br>MEM: ${navigator.deviceMemory || '?'} GB <br>UA: ${navigator.userAgent}`, 'system');
        } else if(c === '/ping') {
             logToConsole("PONG (Local Loopback)", 'system');
        } else {
             logToConsole("UNKNOWN COMMAND", 'error');
        }
    }
    
    async function handleAIChat(prompt) {
        logToConsole("...", 'ai'); // Typing indicator selector
        const msgs = document.querySelectorAll('.console-msg.ai');
        const typingMsg = msgs[msgs.length - 1]; // Get last one
        ...
        try {
            ...
            const apiKey = localStorage.getItem(
                provider === 'gemini' ? 'cloud_gemini' : 
                provider === 'anthropic' ? 'cloud_claude' : 
                provider === 'groq' ? 'cloud_groq' :
                'cloud_openai'
            );
            
            if(!apiKey) {
                 throw new Error(`MISSING KEY FOR ${provider.toUpperCase()}`);
            }
            ...
            if(provider === 'openai') {
                ...
                reply = `Provider ${provider} not fully integrated for chat yet.`;
            }

            // Update Typing Indicator
            // Simple parsing again
            reply = reply.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
            reply = reply.replace(/`(.*?)`/g, '<code style="background:rgba(255,255,255,0.1); padding:2px 4px; border-radius:3px;">$1</code>');
            reply = reply.replace(/\n/g, '<br>');

            if(typingMsg) typingMsg.innerHTML = reply;
            scrollToBottom();

        } catch(e) {
            if(typingMsg) typingMsg.innerHTML = "ERR: " + e.message;
            typingMsg.classList.add('error');
            console.error(e);
        }
    }
